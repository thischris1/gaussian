...
if ( !boost::filesystem::exists(name_of_param_file ) )
{
	ERROR("Could not open gaussian.par in ");
	//glLogger.error( boost::current_path().string());
	...
	if 	(!myParser->readFile(name_of_param_file))
		{
			return (-1);
		}
	...
	else
	{
		...
		else
		{
			...
			if (m_impArray.getSize() > 1) {
				m_impArray.writePotentialToFile(std::string("./PotentialArray.dat"));
			}
...
int gaussian::diagonalize (TRESULT & listEigVectors, yosBasis & basis)
{
	...
	*/
	...
	case Operator::SPARSE_COMPLEX:
		...
		break;
	case Operator::SPARSE_COMPLEX_EIGEN: // @suppress("Symbol is not resolved")
		m_bComplex = true;
		nCount = basis.dimension() * 2;

		break;
	}
	std::vector<double> pdEigVec;
	std::vector<double> pdEigVal;

	glLogger.info("ncount =(%d), m_iEigsToFind =(%d), dimension = (%d)",
			nCount, m_iEigsToFind, basis.dimension() );
	unsigned int trialSize = nCount*m_iEigsToFind;

	try
	{

		pdEigVec.reserve(trialSize);
	}
	catch(...)
	{
	...
	try
	{
		pdEigVal.reserve ( trialSize); // this is too much, n_eigsTo>Find should be enough here
	}
	catch(...)
	{
		glLogger.error("Could not allocate memory");
	...
	for (unsigned int i = 0; i < m_iEigsToFind; i++) {
		if (m_bComplex)
		{
